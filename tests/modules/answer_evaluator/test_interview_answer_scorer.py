...
class TestInterviewAnswerScorer(unittest.TestCase):
    ...
    def test_prompt_info_for_multi_prompt(self):
        self.assertTrue(is_not_none_string(self.scorer.prompt))

        prompt_info = self.scorer.prompt
        print(
    ...
    def test_prompt_info_for_router_chain(self):
        ...
        for prompt_info_name in self.scorer.templates_for_prompt_info:
            self.assertTrue(is_not_none_string(prompt_info_name))
            self.assertTrue(is_not_none_string(self.scorer.templates_for_prompt_info[prompt_info_name]))
            
            print("prompt_info_name : ", prompt_info_name)
            print('prompt_info_value', self.scorer.templates_for_prompt_info[prompt_info_name])
            print("----------------------------------------------------")
