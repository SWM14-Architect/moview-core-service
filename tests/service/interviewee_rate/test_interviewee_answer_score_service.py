...
class TestScoreService(unittest.TestCase):
    ...
    def test_score_one_answer_of_interviewee(self):
        ...
        entity.save_category_in_interviewee_answer_scores(question="프로젝트에서 어떤 언어와 프레임워크, 라이브러리를 사용하였나요?",
        ...
        # when
        updated_id = self.score_service.score_answers_of_interviewee(session_id=saved_id)

        # then
        found_entity = self.repository.find_by_session_id(session_id=updated_id)

        self.assertEqual(len(found_entity.interviewee_answer_scores.score_of_answer_list), 1)
        print("\nPRINT\n")
        print("\n", found_entity.interviewee_answer_scores.score_of_answer_list)
    ...
    def test_score_answers_of_interviewee(self):
        ...
        # then
        found_entity = self.repository.find_by_session_id(session_id=updated_id)
        self.assertEqual(len(found_entity.interviewee_answer_scores.score_of_answer_list), ANSWER_COUNT)

        print("\nPRINT\n")
        for i in range(len(found_entity.interviewee_answer_scores.score_of_answer_list)):
            print("\n", found_entity.interviewee_answer_scores.score_of_answer_list[i])
        """
        출력 형태
        (첫 번째 경우)
        - Technical Details
