...
class AnswerService:
    ...
    def answer(self, question_id: str, question_content: str, answer_content_str):
        # 1. answer 서비스에서 answer 필터 로직을 실행한다.
        filter_result = self.filter.exclude_invalid_answer(question=question_content, answer=answer_content_str)

        # 2. answer 필터 로직을 실행한 결과를 지역 변수로 저장한다. (이를 a라 하자. a는 off_topic_response 같은 문자열이 들어 있다.)
        ...
        # 8. random()을 활용하여 0과 1 사이의 랜덤한 실수를 얻는다.
